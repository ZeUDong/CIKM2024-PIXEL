...
def main(config, gpu_transform=False, gpu_mean_transform=False, method='supervised'):
    ...
    # print("attr_data: ",attr_data.shape)
    # _input_size = (3,224,224)
    # stat(model,_input_size,attr_data)
    ...
    for it in range(niters):
        ...
        # # # 创建一个随机的输入张量，大小应与模型期望的输入相匹配  
        # input_data = torch.randn(1, 3, 224, 224).to(device)
        # # 使用thop的profile函数来计算FLOPs和参数数量  
        # flops, params = thop.profile(model, inputs=(input_data, ), verbose=False)  
        # print(f"(224)FLOPs: {flops / 1e9} G")  # 转换为GigaFLOPs  
        # print(f"(224)Params: {params / 1e6} M")  # 转换为Mega参数量
        # print(model.device)
        ...
        for ep in range(config['start_epoch_from'], nepochs):
            ...
            print("train epoch time:",time.time()-t)
